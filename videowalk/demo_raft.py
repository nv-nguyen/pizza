...
#     parser.add_argument('--config_raft', help='Config file of RAFT model')
#     parser.add_argument('--config_run', help='Config file of run')
...
class SingleVideo(object):
    def __init__(self, config_raft, root_path, max_num_mask_per_frame, nms_thre, mask_matching_thre, patience):
        self.load_rgb_path_and_mask(root_path)
        self.mask_per_frame = max_num_mask_per_frame
    ...
    def load_rgb_path_and_mask(self, root_path):
        dict_rgb_to_masks = {}
        mask_video = json.load(open(os.path.join(root_path, "box2seg.json"), 'r'))
        for mask_frame in mask_video:
            rgb_path = mask_frame["file_name"]
            mask = transform_mask.decode(mask_frame['segmentation'])
            score = mask_frame["score"]
            if rgb_path not in dict_rgb_to_masks:
                dict_rgb_to_masks[rgb_path] = {"masks": [mask], "scores": [score]}
            else:
                dict_rgb_to_masks[rgb_path]["masks"].append(mask)
                dict_rgb_to_masks[rgb_path]["scores"].append(score)
        self.images_path, self.masks, self.scores = [], [], []
        for rgb_path in dict_rgb_to_masks:
            self.images_path.append(rgb_path)
            self.masks.append(dict_rgb_to_masks[rgb_path]["masks"])
            self.scores.append(dict_rgb_to_masks[rgb_path]["scores"])
        print(f"Found {len(self.images_path)} RGB!")
...
if __name__ == '__main__':
    ...
    config_tracking = Config("./configs/config_tracking.json").get_config()
    config_run = Config("./configs/config_proposals.json").get_config()
    config_tracking.raft.opflow_model_path = os.path.join(config_run.weight_dir, "raft-things.pth")
    
    # reset args
    list_video = get_list_video(config_run.input_dir)
    for video_path in tqdm(list_video):
        video_name = os.path.basename(video_path)
        single = SingleVideo(config_raft=config_tracking.raft,
                            root_path=os.path.join(config_run.output_dir, video_name),
                            max_num_mask_per_frame=config_tracking.tracking.max_num_mask_per_frame,
                            nms_thre=config_tracking.tracking.nms_thre,
                            mask_matching_thre=config_tracking.tracking.mask_matching_thre,
                            patience=config_tracking.tracking.patience)
        single.inference()
        ins = single.format_results(os.path.join(config_run.output_dir, "tracking_results"), video_name)
        del single
