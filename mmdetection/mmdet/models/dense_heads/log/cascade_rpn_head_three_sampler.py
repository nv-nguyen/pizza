...
from mmdet.core.bbox.assigners.atss_loose_assigner import ATSSLooseAssigner
...
class StageCascadeRPNHead(RPNHead):
    ...
    def __init__(self,
        ...
        if self.train_cfg:
            ...
            if self.train_cfg.iou_assigner is not None:
            ...
            if self.train_cfg.iou_sampler is not None:
                self.iou_sampler = build_sampler(self.train_cfg.iou_sampler, context=self)
    ...
    def get_iou_atss_targets(self,
        ...
        (all_anchors, all_labels, all_label_weights, all_bbox_targets,
         all_bbox_weights, pos_inds_list, neg_inds_list) = multi_apply(
             self._get_iou_atss_target_single,
             concat_score_list,
             concat_anchor_list,
             concat_valid_flag_list,
    ...
    def _get_iou_atss_target_single(self,
                           flat_scores,
                           flat_anchors,
                           valid_flags,
                           num_level_anchors,
        ...
        sampling_result = self.iou_sampler.sample(assign_result, anchors,
                                              gt_bboxes)
    ...
    def get_aux_atss_targets(self,
        ...
        (all_anchors, all_labels, all_label_weights, all_bbox_targets,
         all_bbox_weights, pos_inds_list, neg_inds_list) = multi_apply(
             self._get_aux_atss_target_single,
             concat_score_list,
             concat_anchor_list,
             concat_valid_flag_list,
             num_level_anchors_list,
    ...
    def _get_aux_atss_target_single(self,
                           flat_scores,
                           flat_anchors,
                           valid_flags,
                           num_level_anchors,
    ...
    def get_targets(self,
        ...
        if isinstance(self.assigner, ATSSAssigner) \
                or isinstance(self.assigner, RPN_SimOTAAssigner) \
                or isinstance(self.assigner, ATSSLooseAssigner):
            cls_reg_targets = self.get_atss_targets(
                ...
                label_channels=label_channels)
            if self.train_cfg.aux_assigner is not None:
                aux_cls_reg_targets = self.get_aux_atss_targets(
                    scores,
                    anchor_list,
                    valid_flag_list,
                    gt_bboxes,
                ...
                aux_cls_reg_targets = None
            if self.train_cfg.iou_assigner is not None:
                iou_cls_reg_targets = self.get_iou_atss_targets(
                    scores,
                    anchor_list,
                    valid_flag_list,
                    gt_bboxes,
            ...
            aux_cls_reg_targets = None
            iou_cls_reg_targets = None
        return cls_reg_targets, aux_cls_reg_targets, iou_cls_reg_targets
    ...
    def loss_iou_atss_single(self, anchors,
                            cls_score, bbox_pred, iou,
                            labels, label_weights, bbox_targets,
                            aux_labels, aux_label_weights, aux_bbox_targets,
                            iou_labels, iou_label_weights, iou_bbox_targets,
                            num_total_samples, aux_num_total_samples, iou_num_total_samples):
        ...
        iou_bbox_targets = iou_bbox_targets.reshape(-1, 4)
        labels = labels.reshape(-1)
        aux_labels = aux_labels.reshape(-1)
        iou_labels = iou_labels.reshape(-1)
        label_weights = label_weights.reshape(-1)
        aux_label_weights = aux_label_weights.reshape(-1)
        iou_label_weights = iou_label_weights.reshape(-1)
        ...
        pos_inds_iou = ((iou_labels >= 0)
                    & (iou_labels < bg_class_ind)
                    & ((aux_labels < 0) | (aux_labels >= bg_class_ind))).nonzero().squeeze(1)
        ...
        if len(pos_inds_iou) > 0:
            pos_bbox_targets = iou_bbox_targets[pos_inds_iou]
            pos_bbox_pred = bbox_pred[pos_inds_iou]
            pos_anchors = anchors[pos_inds_iou]
            pos_iou = iou[pos_inds_iou]
            ...
            # iou loss
            loss_iou = loss_iou + self.loss_iou(
                pos_iou,
                iou_targets,
                avg_factor=iou_num_total_samples)
        else:
            loss_bbox = loss_bbox + bbox_pred.sum() * 0
            loss_iou = loss_iou + iou.sum() * 0

        return loss_cls, loss_bbox, loss_iou, iou_targets.sum()


    def loss_aux_atss_single(self, anchors,
                            cls_score, bbox_pred, iou,
                            labels, label_weights, bbox_targets,
                            aux_labels, aux_label_weights, aux_bbox_targets,
    ...
    def loss_atss_single(self, anchors, cls_score, bbox_pred, iou, labels,
    ...
    def loss(self,
        ...
        cls_reg_targets, aux_cls_reg_targets, iou_cls_reg_targets = self.get_targets(
            scores,
            anchor_list,
            valid_flag_list,
            gt_bboxes,
        ...
        if isinstance(self.assigner, ATSSAssigner) \
                or isinstance(self.assigner, RPN_SimOTAAssigner) \
                or isinstance(self.assigner, ATSSLosseAssigner):
            ...
            if self.train_cfg.iou_assigner is not None:
                (_, iou_labels_list, iou_label_weights_list, iou_bbox_targets_list, iou_bbox_weights_list,
                iou_num_total_pos, iou_num_total_neg) = iou_cls_reg_targets
        else:
        ...
        if self.sampling:
        ...
        elif isinstance(self.assigner, ATSSAssigner) \
                or isinstance(self.assigner, RPN_SimOTAAssigner) \
                or isinstance(self.assigner, ATSSLosseAssigner):
            ...
            if self.train_cfg.iou_assigner is not None:
                iou_num_total_samples = reduce_mean(
                    torch.tensor(iou_num_total_pos, dtype=torch.float,
                                 device=device)).item()
                iou_num_total_samples = max(iou_num_total_samples, 1.0)
        else:
        ...
        #if not isinstance(self.assigner, ATSSAssigner) or not isinstance(self.assigner, RPN_SimOTAAssigner):
        ...
        if self.train_cfg.aux_assigner is not None and self.train_cfg.iou_assigner is None:
            losses_cls, losses_bbox, losses_iou,\
            bbox_avg_factor = multi_apply(
                self.loss_aux_atss_single,
                mlvl_anchor_list,
                cls_scores,
                bbox_preds,
                ious,
        ...
        elif self.train_cfg.aux_assigner is not None and self.train_cfg.iou_assigner is not None:
            losses_cls, losses_bbox, losses_iou,\
            bbox_avg_factor = multi_apply(
                self.loss_iou_atss_single,
                mlvl_anchor_list,
                cls_scores,
                bbox_preds,
                ious,
                ...
                aux_bbox_targets_list,
                iou_labels_list,
                iou_label_weights_list,
                iou_bbox_targets_list,
                num_total_samples=num_total_samples,
                aux_num_total_samples=aux_num_total_samples,
                iou_num_total_samples=iou_num_total_samples)
            bbox_avg_factor = sum(bbox_avg_factor)
            bbox_avg_factor = reduce_mean(bbox_avg_factor).clamp_(min=1).item()
            losses_bbox = list(map(lambda x: x / bbox_avg_factor, losses_bbox))
        else:
    ...
    def _get_bboxes_single(self,
        ...
        for idx in range(len(cls_scores)):
            ...
            if self.use_sigmoid_cls:
                ...
                scores = rpn_cls_score.sigmoid() * rpn_iou.sigmoid()
            else:
