...
class StageCascadeRPNHead(RPNHead):
    ...
    def forward_single(self, x, offset):
        ...
        centerness = self.rpn_centerness(x)
        iou = self.rpn_iou(x)
        bbox_pred = self.rpn_reg(x)
        return bridged_x, cls_score, centerness, iou, bbox_pred
    ...
    def loss_atss_single(self, anchors, cls_score, bbox_pred, centerness, iou, labels,
        ...
        return loss_cls, loss_bbox, loss_centerness, loss_iou, centerness_targets.sum()

    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'centernesses', 'ious'))
    def loss(self,
             anchor_list,
             valid_flag_list,
             cls_scores,
        ...
        if not isinstance(self.assigner, ATSSAssigner):
        ...
        losses_cls, losses_bbox, losses_centerness, losses_iou,\
        ...
        if self.with_cls:
            return dict(
                ...
                loss_rpn_centerness=losses_centerness,
                loss_rpn_iou=losses_iou)
        return dict(loss_rpn_reg=losses_bbox, loss_rpn_centerness=losses_centerness, loss_rpn_iou=losses_iou)
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            proposals = self._get_bboxes_single(cls_score_list, bbox_pred_list, centerness_list, iou_list,
    ...
    def _get_bboxes_single(self,
        ...
        for idx in range(len(cls_scores)):
            ...
            if self.use_sigmoid_cls:
                ...
                scores = rpn_cls_score.sigmoid() * rpn_centerness.sigmoid() * rpn_iou.sigmoid()
            else:
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            if stage.adapt_cfg['type'] == 'offset':
                ...
                offset_list = None
            x, cls_score, centerness, iou, bbox_pred = stage(x, offset_list)
            rpn_loss_inputs = (anchor_list, valid_flag_list, cls_score, centerness, iou,
        ...
        if proposal_cfg is None:
            ...
            proposal_list = self.stages[-1].get_bboxes(anchor_list, cls_score, bbox_pred, centerness, iou,
                                                       img_metas,
                                                       self.test_cfg)
